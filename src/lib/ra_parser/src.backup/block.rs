...
impl<'a> Block<'a> {
    ...
    fn parse_block_kind(token: RaToken<'a>) -> Result<BlockKind, ParserError> {
        ...
        match token_kind {
            ...
            TokenKind::Plus | TokenKind::Greater => {
                Ok(BlockKind::Input(token_kind == TokenKind::Plus, None))
            }
            ...
            BlockKind::Invocation(invoked_token, expression) => {
                if invoked_token.is_none() {
                    match token.kind.unwrap() {
                        ...
                        _ => {
                            return Err(ParserError::ExpectedAGotB(
                                format!("{}", token),
                    ...
                    block.kind =
                        BlockKind::Invocation(invoked_token, Some(InputExpression::new(token)?));
                } else {
                    block.kind = BlockKind::Invocation(
                        invoked_token,
                        Some(expression.unwrap().append_item(token, None)?),
                    )
                }
