    ...
    I: Iterator<Item = Token<'token>>,
{
        ...
        match first_token {
            Some(token) => {
                match token.kind.unwrap() {
                    ...
                    TokenKind::Plus |
                     TokenKind::Greater => self.parse_block(token, BlockKind::Input),
    ...
    fn parse_block (&mut self, first_token: Token<'token>, kind: BlockKind) -> Result<Block<'token>, Vec<ParserError>> {
        let mut errors: Vec<ParserError> = Vec::new();
        let block = Some(Block::from_token_and_kind(first_token, kind));

        if let (Some(blk), mut errs) = self.check_parse_block_expression(block.unwrap(), &mut errors) {
    ...
    fn check_parse_block_expression<'errors>(&mut self, mut block: Block<'token>, errors: &'errors mut Vec<ParserError>) -> (Option<Block<'token>>, &'errors mut Vec<ParserError>) {
        match self.first_ahead() {
            Some(token) => {
                let first_block_token = block.expression.tokens[0];
                if token.level == first_block_token.level 
                && (token.position.0).0 == (first_block_token.position.0).0 {
                    block.expression = Expression::from_token(self.bump().unwrap());
                    while !self.is_eof() 
                    && self.first_ahead().is_some()
                    && (self.first_ahead().unwrap().position.0).0 == (first_block_token.position.0).0 {
                        match block.expression.append_token(self.bump().unwrap()) {
                            Some(errs) => {
                                errors.extend(errs)
                            },
                            None => {}
    ...
    fn check_parse_block_children<'errors>(&mut self, mut block: Block<'token>, errors: &'errors mut Vec<ParserError>) -> (Option<Block<'token>>, &'errors mut Vec<ParserError>) {
        let first_block_token = block.expression.tokens[0];
        while !self.is_eof()
        && self.first_ahead().unwrap().level == first_block_token.level + 1 {
    ...
    fn check_parse_union<'errors>(&mut self, block: Block<'token>, errors: &'errors mut Vec<ParserError>) -> (Option<Block<'token>>, &'errors mut Vec<ParserError>) {
        let first_block_token = block.expression.tokens[0];
